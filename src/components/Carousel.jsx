...
/* SHOTS was Bencho's own pictures, which are not licensed
   ...
   This project ships ONE portrait right now — dika.png — so
   the ring is a single card: it drifts at rest and presses
   under the pointer, but it has nowhere to go when swiped.
   The geometry is sized and tuned for five (see the note on
   `N`); drop more photographs into src/assets/ and extend the
   array — the ring will pick up each one. */
const SHOTS = [
  { name: "one", src: one },
  { name: "two", src: two },
...
function useSpring(target, tune = 50, instant = false) {
  ...
  useEffect(() => {
    if (instant) {
      cur.current = target;
      vel.current = 0;
      /* the instant branch snaps the spring in place — under
         reduced motion nothing animates, so target IS the
         state. The deliberate exception to the rule above. */
...
// eslint-disable-next-line no-unused-vars -- the motion module's bezier helper, kept whole so later blocks can use it
...
/* ── one card, and the press is its own ───────────────────
   ...
   runs no loop at all. */
function Card({ shot, corner, sink, off }) {
